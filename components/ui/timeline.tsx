...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      className='w-full bg-indigo-100 dark:bg-neutral-950 bg-opacity-90 dark:bg-opacity-50 font-sans md:px-10 rounded-lg'
      ref={containerRef}
    >
      <div className='max-w-7xl mx-auto pt-8 sm:pt-20 px-8 md:px-8 lg:px-10'>
        <h2 className='text-4xl md:text-5xl mb-4 text-black font-bold dark:text-white max-w-4xl'>
          Experience
        </h2>
      ...
      <div ref={ref} className='relative max-w-7xl mx-auto pb-20'>
        {data.map((item, index) => (
          <div
            key={index}
            className='flex justify-start pt-10 lg:pb-28 lg:gap-10'
          >
            <div className='sticky flex flex-col lg:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm'>
              ...
              <h3 className='hidden lg:block text-xl lg:pl-20 lg:text-3xl font-bold text-neutral-900 dark:text-neutral-100 '>
                {item.title}
              </h3>
            </div>

            <div className='relative pl-20 pr-4 lg:pl-4 w-full'>
              <h3 className='lg:hidden block text-2xl mb-4 text-left font-bold text-neutral-900 dark:text-neutral-100'>
                {item.title}
              </h3>
              {item.content}
            </div>
